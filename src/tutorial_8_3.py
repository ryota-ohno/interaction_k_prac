...
def init_process(args):
    ...
    def get_init_para_csv(auto_dir,monomer_name):##初期値パラメータ決定
        ...
        theta_list = [round(i/2, 1) for i in range(60, 61)]
        #################################################################
    ...
    if not os.path.exists(auto_csv_path):        
        df_E_init = pd.DataFrame(columns = ['a','b','theta','E','E_p','E_t','machine_type','status','file_name'])##step1.csvに列の名前を指定
    else:
        df_E_init = pd.read_csv(auto_csv_path)
        df_E_init = df_E_init[df_E_init['status']!='InProgress']##statusがinprogressでない部分(終わってない部分)だけ抜き出す　statusはinprogressとnotyet
...
def listen(args):
    ...
    for idx,row in zip(df_queue.index,df_queue.values):##rowはdf_queueのvalue部分 上よりこれはmachine_typeとfilename
        ...
        if len(E_list)!=2:##len(Elist)が2の時
            continue
        else:
    ...
    if isAvailable:
        ...
        if len(params_dict)!=0:#終わりがまだ見えないなら
            ...
            if not(alreadyCalculated):
                file_name = exec_gjf(auto_dir, monomer_name, {**params_dict,'cx':0,'cy':0,'cz':0,'A1':0.,'A2':0.}, machine_type,isInterlayer=False,isTest=isTest)
    ...
    isOver = True if len(df_init_params_done)==len(df_init_params) else False##ここでisOverがTrueになったら終わり
    return isOver
